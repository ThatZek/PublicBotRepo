...
module.exports.run = async (client, msg, args) => {
    ...
    const returnData = await raid.startAFK(raidLeader, afkMsg.id, 'Void');
    await afkMsg.edit("", {
        embed: {
            title: `Void started by ${msg.member.nickname} in Raiding ${channelNumber}`,
            color: 9442302,
            ...
            description: `React with the void icon if you are going to join the run\n
            React with the class's you are willing to bring to the run\n
            Key and rushers are appreciated,Thank you.`
        }
    }).then(async m => {
        const filter = (reaction, user) => reaction.emoji.id === client.reactions.get('Void').id && !user.bot || reaction.emoji.id === client.reactions.get('lhkey').id && !user.bot || reaction.emoji.id === client.reactions.get('nitro').id && !user.bot
        const collector = m.createReactionCollector(filter);
        raid.setCollector(collector);
        collector.on('collect', async r => {
            if (r.emoji.id === client.reactions.get('Void').id) {
            ...
            } else if (r.emoji.id === client.reactions.get('vial').id) {
                user = r.users.cache.last();
                const confirmationMsg = await user.send('Are you sure you have a vial?');
                const filter = (reaction, user) => reaction.emoji.id === client.reactions.get('vial').id && !user.bot
                confirmationMsg.react(client.reactions.get('vial'));
                const collector = confirmationMsg.createReactionCollector(filter, { maxUsers: 1 });
                collector.on('collect', async r => {
                if (r.emoji.id === client.reactions.get('vial').id) {
                user = r.users.cache.last();
                await user.send('The location is: ' + raid.getLocation());
                await collector.stop();
                raid.addVial(user.id);
                }
                });
        ...
        await m.react(client.reactions.get('Void'));
        await m.react(client.reactions.get('lhkey'));
        await m.react(client.reactions.get('vial'));
        ...
        await m.react(client.reactions.get('Wizard'));
        ...
        await m.react(client.reactions.get('nitro'));
    });
    return msg.delete();
}
module.exports.help = {
    ...
    wlRoles: [arl, rl],
    syntax: `void {channel} {Server Location} {Bazzar Location}`
}